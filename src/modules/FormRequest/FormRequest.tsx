...
export function FormRequest() {
    ...
    const initialValues: { name: string, email: string, products: string[], privacy: boolean } = { name: '', email: '', products: [], privacy: false }
    ...
    useEffect(() => {
        formik.setFieldValue('products', isHas('userSelect') ? (query.userSelect as string).split(',') : [])
    }, [query.userSelect])

    return (
        <form onSubmit={formik.handleSubmit} onClick={(e) => e.stopPropagation()} className={s.FormRequest}>
            <Title
            ...
            <div className={s.FormRequest__input}>
                <Input text="Имя" type="text" name='name' placeholder="Имя" autoComplete="off" value={formik.values.name} onChange={formik.handleChange} />
                <Input text="Электронная почта" type="text" placeholder="Email" autoComplete="off" name='email' value={formik.values.email} onChange={formik.handleChange} />
            </div>
